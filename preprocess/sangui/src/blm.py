...
class BLM():
    def __init__(self, config):
        ...
        self.output_dir = config["blm"]["model_output_dir"]
        self.model_name = config["blm"]["model_name"]
        print("blm model_name", self.model_name)
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_name)
        self.model = AutoModelForSequenceClassification.from_pretrained(self.model_name, num_labels=7)
        self.device = config["device"]

        self.batch_size = config["blm"]["batch_size"]
        ...
        test_data = pd.read_csv(config["data_folder"]+'test.csv')
    ...
    def test(self, data=None):
        ...
        dataset_test.to_csv('../output/output.csv', index=False)
