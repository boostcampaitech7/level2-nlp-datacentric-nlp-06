...
class LlamaTrainer(LlamaModel):
    def __init__(self, model_name, data_path, prompt_path):
    ...
    def prepare_dataset(self):
        ...
        def process_data(dataframe):
            ...
            for _, row in dataframe.iterrows():
                title = row['text']
                text = prompt_template.format(title=title)
                texts.append(text)
...
class LlamaInference(LlamaModel):
    def __init__(self, model_name, prompt_path, adapter_path=None):
        super().__init__(model_name)
        self.load_tokenizer()
        self.load_model()
    ...
    def inference(self, input_text):
        prompt = self.prompt_template.format(title=input_text)
...
def main():
    ...
    if args.mode == "train":
        trainer = LlamaTrainer(args.model_name, args.data_path, args.prompt_path)
        trainer.train()
    elif args.mode == "inference":
        inferencer = LlamaInference(args.model_name, args.prompt_path, args.adapter_path)
        while True:
